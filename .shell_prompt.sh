...
function __promptline_ps1 {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "z" slices
  __promptline_wrapper "%m" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline_right_prompt {
  ...
  # section "z" slices
  __promptline_wrapper "%m" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
...
function __promptline {
  ...
  local x_bg="${wrap}48;5;67${end_wrap}"
  local x_sep_fg="${wrap}38;5;67${end_wrap}"
  ...
  local z_bg="${wrap}48;5;144${end_wrap}"
  local z_sep_fg="${wrap}38;5;144${end_wrap}"
  if [[ -n ${ZSH_VERSION-} ]]; then
